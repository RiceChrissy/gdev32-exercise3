 ...
 * IJKL - Spotlight X & Z Position Movement
...
float spotLighOuterRadius = 0;                                          // in degrees
bool attenuationIsOn = false;
float deg = 30.0f;
...
bool setupShadowMap()
{
    ...
    shadowMapShader = gdevLoadShader("demo5s.vs", "demo5s.fs");
    if (!shadowMapShader)
        return false;
...
void render()
{
    ...
    {
        spotLightDirection += lightForward * 0.01f;
    }
    ...
    {
        spotLightDirection -= glm::cross(lightForward, glm::vec3(0.0f, 1.0f, 0.0f)) * 0.01f;
    }
    ...
    {
        if (deg >= 60 || deg >= outerDeg - 0.1f)
            deg = deg - 0.1f;
        else
            deg += 0.1f;
        spotLightRadius = cos(glm::radians(deg));
    }
    ...
    if (glfwGetKey(pWindow, GLFW_KEY_UP) == GLFW_PRESS)
    {
        if (outerDeg >= 65)
            ...
            outerDeg += 0.1f;
        spotLightOuterRadius = cos(glm::radians(outerDeg));
    }
    if (glfwGetKey(pWindow, GLFW_KEY_DOWN) == GLFW_PRESS)
    {
        if (outerDeg <= 15 || outerDeg <= deg + 0.1f)
            outerDeg = outerDeg + 0.1f;
        else
            outerDeg -= 0.1f;
        spotLightOuterRadius = cos(glm::radians(outerDeg));
    }
